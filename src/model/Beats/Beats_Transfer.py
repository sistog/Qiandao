...
from .BEATs import BEATs, BEATsConfig, IMUFeatureExtractor, FocalLossMulti
...
class BEATsTransferLearningModel(pl.LightningModule):
    def __init__(
        ...
        num_workers: int = 6,
        model_path: str = "/data/zcx/wav_prj/v1/Beats_Model/model/BEATs_iter3_plus_AS20K.pt",
    ...
    def forward(self, x, padding_mask=None):
        ...
        # 需要梯度：正常计算
            ...
            # 不需要梯度：使用torch.no_grad()避免计算开销
            with torch.no_grad():  # 🔥 关键改进
                if padding_mask != None:
                    x, _ = self.beats.extract_features(x, padding_mask)
                else:
                    x, _ = self.beats.extract_features(x)
